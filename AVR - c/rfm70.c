...
///////////////////////////////////////////////////////
// Makra komend i warto�ci inicjalizuj�ce rejestry	//
//////////////////////////////////////////////////////
...
const uint8_t PROGMEM RFM70_bank0Init[][2] = {
	...
	{ (0x20|0x01), 0x00 }, //Enable auto acknowledgement data pipe0-5
	{ (0x20|0x02), 0x3F }, //Enable RX Addresses pipe0-5
	{ (0x20|0x03), 0x03 }, //RX/TX address field width 5byte
	{ (0x20|0x04), 0x10 }, //x = 250 ms = 4000ms, y = 15 tries xxx08
	{ (0x20|0x05), 0x17 }, //channel = 0x17
	{ (0x20|0x06), 0x17 }, //air data rate-2M,out power 5dbm,setup LNA gain high (0dBM) /novik?? zamiast 3f chyba 2f
	{ (0x20|0x07), 0x07 }, //
	{ (0x20|0x08), 0x00 }, //
	{ (0x20|0x09), 0x00 }, //
...
const uint8_t PROGMEM RFM70_bank1Init[][5] = {
	...
	{ (0x20|0x04), 0xD9, 0x96, 0x82, 0x1B }, // b9? f9? // tu chyba te�, ale jest odwrotnie??
	//{ (0x20|0x04), 0xb9, 0x9E, 0x86, 0x0B },
	...
	{ (0x20|0x0D), 0x46, 0xb4, 0x80, 0x00 } //novik tu zmie� wg dokumentacji
};
...
void initRFM() //inicjalizacja sprz�tu
{
	#ifdef USART_LIB
	USART_Init();
	#endif
	
	// pin Chip Enable - wyj�cie i stan niski
	DDR_SPI |= (1<<CE);
	PORT_SPI &=~(1<<CE);
	
	//ustawiamy diod� informacyjn� 
	DDR_BLINK |= (1<<RED);
	DDR_BLINK |= (1<<GREEN);
	blinker(0b10100000);
	...
	_delay_ms(RFM70_BEGIN_INIT_WAIT_MS);//koniecznie op�nienie - datasheet
	blinker(0b10100000);
	initRegisters(); // inicjalizujemy rejestry

}

void initRegisters(void) //inicjalizacja rejestr�w
{
	// wybieramy bank 0
	selectBank(0);

	//przepisujemy warto�ci inicjalizacyjne
	for (int i = 0; i < 20; i++)
	...
	if (readRegVal(0x08) != 0x63) // sprawdzamy poprawno�� inicjalizacji przez odczyt ID
		debug(0x01);		
	else
		debug(0x03);

	//wybieramy bank 0 i domy�lnie ustawiamy odbiornik w tryb RX
	selectBank(0);
...
uint8_t transmitSPI(uint8_t val) //funkcja przesy�ania danych przez sprz�towy interfejs SPI
{
	SPDR = val;
	while (!(SPSR & _BV(SPIF)))
	;
...
void selectBank(uint8_t bank) //wyb�r banku ustawie�
{
...
{
	uint8_t val;
	writeRegPgmBuf((uint8_t *)RFM70_cmd_flush_rx, sizeof(RFM70_cmd_flush_rx)); // opr�niamy bufor
	val = readRegVal(RFM70_REG_STATUS); // odczytujemy status
	writeRegVal(RFM70_CMD_WRITE_REG | RFM70_REG_STATUS, val); //reset bit�w przerwa�
...
uint8_t readRegVal(uint8_t cmd)  // odczytaj warto�� z rejestru
{
	uint8_t res;
	PORT_SPI &=~ (1<<CSN);
...
uint8_t writeRegVal(uint8_t cmd, uint8_t val) // zapisz warto�� do rejestru
{
	PORT_SPI &=~ (1<<CSN);
	_delay_ms(RFM70_CS_DELAY);
	transmitSPI(cmd);
...
void readRegBuf(uint8_t reg, uint8_t * buf, uint8_t len) //odczytaj tablic� z rejestru
{
...
uint8_t writeRegPgmBuf(uint8_t * cmdbuf, uint8_t len) // zapisz tablic� do rejestru
{
	PORT_SPI &=~ (1<<CSN);
	_delay_ms(RFM70_CS_DELAY);
...
uint8_t writeRegCmdBuf(uint8_t cmd, uint8_t * buf, uint8_t len) //zapisz komend� do rejestru
{
	PORT_SPI &=~ (1<<CSN);
	_delay_ms(RFM70_CS_DELAY);
	transmitSPI(cmd);
...
//////////////////////////////////////////////////////////////////////////////////
// funkcje odpowiedzialne za wysy�anie i odbieranie danych oraz obs�ug� bufor�w //
//////////////////////////////////////////////////////////////////////////////////
