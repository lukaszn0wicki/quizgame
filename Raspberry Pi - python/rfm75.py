...
from rfm75_defs import *

CSN = 19  # 24 #GPIO pinu CSN
CE = 6  # 7 #GPIO pinu CE
IRQ = 26  # 25 #GPIO pinu IRQ
spi = spidev.SpiDev()  # inicjalizacja SPI

RX0_Address = [0x34, 0x43, 0x10, 0x10, 0x01]  # adres urzadzenia

bank0_vals = [  # ustawienia modulu
    [0, 0x0F],  # receive, enabled, CRC 2, enable RX MAX_RT interupts
    [1, 0x00],  # auto-ack on all pipes enabled
    [2, 0x3F],  # Enable pipes 0 and 1
    [3, 0x03],  # 5 bytes addresses
    [4, 0x10],  # auto retransmission delay 4000 ms, 15 times
    [5, 0x08],  # channel 10
    [6, 0x17],  # data rate 1Mbit, power 5dbm, LNA gain high, 2F default
    [7, 0x07],  # nie ruszac
...
def set_channel(chan):
    register_write(RFM73_REG_RF_CH, chan)


def read_channel():
    return (register_read(RFM73_REG_RF_CH))
...
def init(callback):
    ...
    spi.max_speed_hz = 4000000  # 7629#7800000
...
def receive():
    ...
    return buffer_read(RFM73_CMD_R_RX_PAYLOAD, 1)  # len)
